 ...
 * 
 * This test script verifies that email verification emails can be sent using Resend.
 ...
 */
async function testEmailVerification(): Promise<{ success: boolean, id?: string, error?: unknown }> {
  ...
  try {
    const result = await resend.emails.send({
      ...
      react: EmailVerification({ name: TEST_NAME, verificationLink }),
    });
    ...
    console.log('3. The email verification flow can send emails');
...
describe('Email Verification', () => {
  ...
  it.skip('can send verification emails', async () => {
